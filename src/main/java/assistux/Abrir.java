...
public class Abrir {
    public static void abrir(String texto) {	//metodo que realiza la busqueda
    	String[] vAbrir = {"abre", "abrir"};	//vuelve a declarar el arreglo de strings con las variaciones de busqueda
		...
		texto = texto.trim();
		String[] sitio = texto.split("\\.");
		String[] extensiones = {"com", "mx", "io", "udg", "org", "net", "co", "us"};
		for(int i = 0; i < extensiones.length; i++)
			if(sitio[1].equalsIgnoreCase(extensiones[i])) {
		        String url = "http://"+texto;	//Arma la busqueda
